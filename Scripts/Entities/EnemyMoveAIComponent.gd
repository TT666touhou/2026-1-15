...
func _execute_move_logic() -> void:
	...
	# 1. 檢查是否已經對齊 (8方向對齊：同一列、同一行、或對角線)
	var is_aligned = (dx == 0 or dy == 0 or abs(dx) == abs(dy))
	...
	if is_aligned:
		...
		# 2. 未對齊，優先嘗試斜向移動來同時縮短 X 與 Y 的距離
		if dx != 0 and dy != 0:
			move_dir = Vector2i(sign(dx), sign(dy))
			_reason = "Not aligned, moving diagonally to close both axes (dx=%d, dy=%d)." % [dx, dy]
		elif dx != 0:
			move_dir = Vector2i(sign(dx), 0)
			_reason = "Not aligned, moving on X-axis."
		else:
			move_dir = Vector2i(0, sign(dy))
			_reason = "Not aligned, moving on Y-axis."
...
func _update_attack_direction(p_pos: Vector2i, e_pos: Vector2i) -> void:
	...
	if abs(dx) == abs(dy) and dx != 0:
		new_dir = Vector2i(sign(dx), sign(dy))
	# 其次判定軸向對齊
	elif dx == 0 and dy != 0:
