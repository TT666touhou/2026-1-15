...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and event.keycode == KEY_F1:
		visible = !visible
		print("[SkillSwapperUI] Toggle visibility: ", visible)
...
func _on_skill_list_pressed(skill: UnitSkillData) -> void:
	...
	print("[SkillSwapperUI] Selected skill: ", skill.skill_name)
...
func _on_slot_pressed(index: int) -> void:
	if not selected_skill or not is_instance_valid(current_target_unit): 
		print("[SkillSwapperUI] Cannot swap: No skill selected or no unit found.")
		return
	...
	print("[SkillSwapperUI] Swapped Slot %d to %s" % [index + 1, selected_skill.skill_name])
	_update_slots()
